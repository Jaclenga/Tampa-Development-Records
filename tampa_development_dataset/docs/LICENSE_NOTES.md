...
The v0.6.0 public archive is City-only and source-bounded. It excludes the HCPA archive, extracted
DBF, and HCPA nearest-centroid fallback rows. An optional local HCPA-enriched
build remains available for research, but should not be redistributed until
the current HCPA terms are confirmed.
